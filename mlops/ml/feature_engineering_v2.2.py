...
def build_student_snapshot(
    ...
    snapshot["academic_period_key"] = academic_period_key
...
def build_academic_model_snapshot(
    ...
    existing_count_cols = [
        ...
        if col in academic_snapshot.columns
    ]
    if existing_count_cols:
        academic_snapshot[existing_count_cols] = (
            academic_snapshot[existing_count_cols]
...
# for moodle online prediction
def build_behaviour_prediction_snapshot(student_snapshot):
    """
    Build behaviour snapshot for online prediction.
    No risk_label because current semester has no final outcome yet.
    """
...
def build_academic_prediction_snapshot(student_snapshot):
    """
    Build academic snapshot for online prediction.
    No risk_label and do not remove No Result students.
    """
    ...
    academic_snapshot = snapshot.drop(
        columns=[col for col in drop_cols if col in snapshot.columns]
    )
...
def filter_prediction_snapshot(
        snapshot,
        academic_period_key,
        course_key=None
): 
    ...
    if course_key is not None:
        filtered_snapshot = filtered_snapshot[
            filtered_snapshot["course_key"] == course_key
        ].copy()
